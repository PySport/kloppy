...
class TestSignalityTracking:
    ...
    def test_correct_deserialization(
        ...
        assert dataset.metadata.periods[0].start_timestamp == datetime(2024, 10, 6, 12, 0, 1, 408000, timezone.utc)
        assert dataset.metadata.periods[0].end_timestamp == datetime(2024, 10, 6, 12, 48, 43, 858000, timezone.utc)
        assert dataset.metadata.periods[1].id == 2
        assert dataset.metadata.periods[1].start_timestamp == datetime(2024, 10, 6, 13, 4, 45, 775000, timezone.utc)
        assert dataset.metadata.periods[1].end_timestamp == datetime(2024, 10, 6, 13, 54, 11, 730000, timezone.utc)
        ...
        assert start_frame_home_team_gk_data.coordinates == Point(x=-47.78, y=0.59)
        ...
        assert start_frame_away_team_gk_data.coordinates == Point(x=46.48, y=0.23)
        assert start_frame_away_team_gk_data.speed == 0.942

        assert start_frame.ball_coordinates == Point3D(x=-41.18, y=0.08, z=0)

        first_second_half_frame = next(frame for frame in dataset.frames if frame.period.id == 2)
        assert first_second_half_frame.frame_id == 73063

        first_second_half_frame_home_team_gk_data = first_second_half_frame.players_data[home_team_gk]
        assert first_second_half_frame_home_team_gk_data.coordinates == Point(x=45.37, y=0.22)
