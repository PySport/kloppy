...
def _load_periods(metadata_elm, team_map: dict, frame_rate: int) -> Tuple[List[Period], AttackingDirection]:
    global_config_elm = metadata_elm.find("GlobalConfig")
    provider_params = _load_provider_parameters(global_config_elm.find("ProviderGlobalParameters"))
    ...
    for idx, period_name in enumerate(period_names):
        ...
        if start_key in provider_params:
            periods.append(
                Period(
                    id=idx + 1,
                    start_timestamp=timedelta(seconds=float(provider_params[start_key]) / frame_rate),
                    end_timestamp=timedelta(seconds=float(provider_params[end_key]) / frame_rate),
                )
            )
        else:
            # done
...
def _load_players(players_elm, team: Team) -> List[Player]:
    return [
        Player(
            ...
            starting_position=_load_position_data(player_elm.find("ProviderPlayerParameters")),
            attributes=_load_provider_parameters(player_elm.find("ProviderPlayerParameters")),
...
def _load_position_data(parent_elm) -> PositionType:
    ...
    if "position_index" not in player_provider_parameters:
        ...
        position_type = position_types_mapping.get(player_provider_parameters["position_index"], PositionType.Unknown)
...
def _load_data_format_specifications(
    ...
    return [
        ...
        for data_format_specification_elm in data_format_specifications_elm.iterchildren(tag="DataFormatSpecification")
    ]


def _load_sensors(sensors_elm) -> List[Sensor]:
    return [Sensor.from_xml_element(sensor_elm) for sensor_elm in sensors_elm.iterchildren(tag="Sensor")]


def _load_pitch_dimensions(metadata_elm, sensors: List[Sensor]) -> Union[None, PitchDimensions]:
    normalized = False
...
def _load_provider(metadata_elm, provider: Optional[Provider] = None) -> Optional[Provider]:
...
def load_metadata(metadata_file: IO[bytes], provider: Optional[Provider] = None) -> EPTSMetadata:
    ...
    if score_path.hasattr(metadata):
        ...
        score = Score(home=score_elm.LocalTeamScore, away=score_elm.VisitingTeamScore)
    ...
    for team_id, ground in _team_map.items():
        team = Team(team_id=team_id, name=_team_name_map[team_id], ground=ground)
    ...
    _channel_map = {channel.channel_id: channel for sensor in sensors for channel in sensor.channels}

    _all_players = [player for value in teams_metadata.values() for player in value.players]
    ...
    player_channels = [
        ...
        for player_channel_elm in metadata.find("PlayerChannels").iterchildren(tag="PlayerChannel")
    ]

    frame_rate = int(metadata.find("GlobalConfig").find("FrameRate"))
    pitch_dimensions = _load_pitch_dimensions(metadata, sensors)
    periods, start_attacking_direction = _load_periods(metadata, _team_map, frame_rate)

    if start_attacking_direction != AttackingDirection.NOT_SET:
        orientation = (
            Orientation.HOME_AWAY if start_attacking_direction == AttackingDirection.LTR else Orientation.AWAY_HOME
        )
    else:
