...
class DatasetTransformer:
    ...
    def transform_frame(
        self, frame: Frame, transform_ball_coordinates: bool = True
    ) -> Frame:
        # Change coordinate system
        if self._needs_coordinate_system_change:
            frame = self.__change_frame_coordinate_system(
                frame, transform_ball_coordinates=transform_ball_coordinates
            )

        # Change dimensions
        elif self._needs_pitch_dimensions_change:
            frame = self.__change_frame_dimensions(
                frame, transform_ball_coordinates=transform_ball_coordinates
            )
        ...
        if self._needs_orientation_change:
            if self.__needs_flip(
                ...
                frame = self.__flip_frame(
                    frame,
                    transform_ball_coordinates=transform_ball_coordinates,
                )

        return frame

    def __change_frame_coordinate_system(
        self, frame: Frame, transform_ball_coordinates: bool = True
    ):
        return Frame(
            # doesn't change
            ...
            if transform_ball_coordinates
            else frame.ball_coordinates,
