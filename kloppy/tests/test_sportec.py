...
class TestSportecTrackingData:
    ...
    def test_long_layout_df(self, raw_data: Path, meta_data: Path):
        dataset = sportec.load_tracking(
            raw_data=raw_data,
            meta_data=meta_data,
            coordinates="sportec",
        ...
        long_df = dataset.to_df(layout="long")

        assert long_df.frame_id.nunique() == 100
        assert len(long_df) == 256
        ...
        assert len(long_df.dropna(subset=["z"])) == 100
        assert len(long_df[long_df["team_id"] == "ball"]) == 100
        assert long_df.player_id.nunique() == 4
